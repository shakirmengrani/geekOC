...
class ModelCatalogTestimonial extends Model {
    ...
    public function getTestimonials($start = 0, $limit = 10) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX ."testimonial t LEFT JOIN " . DB_PREFIX . "testimonial_description td ON (t.testimonial_id = td.testimonial_id) WHERE t.status = '1' ORDER BY t.sort_order ASC LIMIT " . (int)$start . "," . (int)$limit);
        return $query->rows;
    }
