		...
		foreach ($menu_items as $item)
		{
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'LINK': 
					...
					$span="";
					$pos1=strpos($link['link'],$labelbest);
					$pos2=strpos($link['link'],$labelnew);
					if($pos1!==false){
						$span="hot";
					}
					if($pos2!==false){
					...
					$this->_menuLink .= '<div id ="pt_menu_link'.$link['cmsblock_des_id'].'" class ="pt_menu pt_menu_link"><div class="parentMenu" ><span class="'.$span.'">'.$span.'</span><a href="'.($link['link']).'"><span>'.$link['title'].'</span></a></div></div>'.PHP_EOL;	
				case 'CMS': 
